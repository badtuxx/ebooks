...
Este e-book é uma contribuição à comunidade open source de Kubernetes, produzido e distribuído gratuitamente em português pela LINUXtips. Esta edição beta aborda dois componentes essenciais do Kubernetes: Secrets e ConfigMaps. A versão final 1.0 será lançada em breve - para se manter atualizado, acompanhe o site da LINUXtips ou inscreva-se na lista de e-mails do Jeferson Fernando.

Este trabalho está licenciado sob GPL-3 e pode ser usado e distribuído respeitando as condições dessa licença. O e-book é um projeto colaborativo e convida você a contribuir com correções ou adições de novos conteúdos no repositório do GitHub:[ https://github.com/badtuxx/ebooks](https://github.com/badtuxx/ebooks). Junte-se a nós nesse esforço para disseminar o conhecimento!

<h3>Resumo do Conteúdo</h3>


Este e-book fornece um guia abrangente para gerenciar Secrets e ConfigMaps no Kubernetes. Os Secrets são um recurso que permite gerenciar informações sensíveis como senhas, tokens OAuth, chaves ssh, entre outros. Aprendemos a criar, obter, descrever e deletar Secrets, e a proteger essas informações críticas usando codificação base64.

O e-book vai além, apresentando o uso de Secrets para armazenar certificados TLS e chaves privadas, configurando o Nginx para usar HTTPS. Aprenderemos como definir Secrets através de um arquivo de manifesto e como usá-los em um Pod do Nginx.

Também exploramos ConfigMaps, que permitem separar parâmetros de configuração das imagens do container, permitindo a mesma imagem ser executada em diferentes ambientes - como desenvolvimento, teste e produção - com configurações distintas. Vamos abordar como atualizar um ConfigMap, definir variáveis de ambiente para os containers no Pod, tornar ConfigMaps imutáveis, e criar um arquivo de configuração do Nginx usando um ConfigMap.

Por fim, este e-book demonstra como usar ConfigMaps e Secrets juntos para configurar um Pod do Nginx para usar HTTPS. Este material detalhado busca equipar os leitores com o conhecimento necessário para gerenciar eficientemente suas configurações e dados sensíveis, proporcionando flexibilidade e controle precisos sobre suas aplicações.

Esperamos que você encontre valor neste material e o incentive a colaborar, melhorando ainda mais este e-book para a comunidade Kubernetes.

<h3>Use, Colabore e Compartilhe!</h3>


Este trabalho foi desenvolvido para disseminar conhecimento e incentivar a colaboração. Esperamos que você encontre valor nele e se sinta motivado a compartilhar o que aprendeu, ensinando outros e fortalecendo a comunidade. 
...
Sim, essas duas peças são fundamentais para que você possa ter a sua aplicação rodando no Kubernetes da melhor forma possível, pois elas são responsáveis por armazenar as informações sensíveis da sua aplicação, como por exemplo, senhas, tokens, chaves de acesso, configurações, etc.
...
* kubernetes.io/dockercfg e kubernetes.io/dockerconfigjson - são usados para armazenar credenciais de registro do Docker. Eles são usados para autenticar Pods com um registro do Docker e são montados em Pods que usam imagens de container privadas.
...
* bootstrap.kubernetes.io/token - são usados para armazenar tokens de inicialização de cluster. Eles são usados para nos autenticar com o control plane do Kubernetes.

Tem mais alguns tipos de Secrets, mas esses que te passei são os mais comuns. Você pode encontrar uma lista completa de tipos de Secrets na documentação do Kubernetes.
...
<h4>
Antes de criar um Secret, o Base64</h4>
...
<h4>
Criando nosso primeiro Secret</h4>
...
Para criar um Secret do tipo Opaque, você precisa criar um arquivo YAML que define o Secret. Por exemplo, você pode criar um arquivo chamado `giropops-secret.yaml` com o seguinte conteúdo:
...
<h4>
Usando o nosso primeiro Secret</h4>


Agora que já temos o nosso primeiro Secret criado, é hora de saber como usá-lo em um Pod.
...
Para usar o Secret em um Pod, você precisa definir o campo `spec.containers[].env[].valueFrom.secretKeyRef` no arquivo YAML do Pod. Eu estou trazendo o campo nesse formato, para que você possa começar a se familiarizar com esse formato, pois você irá usá-lo bastante para buscar alguma informação mais específica na linha de comando, usando o comando `kubectl get`, por exemplo.
...
spec:
  ...
  - name: giropops-container
    image: nginx
    env: # Início da definição das variáveis de ambiente
    - name: USERNAME # Nome da variável de ambiente que será usada no Pod
      valueFrom: # Início da definição de onde o valor da variável de ambiente será buscado
        secretKeyRef: # Início da definição de que o valor da variável de ambiente será buscado em um Secret, através de uma chave
...
Olha lá os nossos Secrets como variáveis de ambiente dentro do container do Pod!
...
<h4>
Criando um Secret para armazenar credenciais Docker</h4>
